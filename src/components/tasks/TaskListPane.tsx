...
const EMPTY_MESSAGES: Record<string, { title: string; message: string }> = {
  inbox: { title: "Inbox is clear", message: "Capture tasks here. They'll stay until you schedule them." },
  today: { title: "Nothing for today", message: "Tasks with today's date (or earlier) show up here." },
  upcoming: { title: "Nothing scheduled", message: "Set an action date on a task to plan ahead." },
  someday: { title: "Nothing in Someday", message: "Tasks you want to revisit eventually but not now." },
  waiting: { title: "Nothing waiting", message: "Tasks blocked on someone or something else." },
  logbook: { title: "No completed tasks", message: "Finished tasks appear here." },
};
...
export function TaskListPane() {
  ...
  const empty = EMPTY_MESSAGES[selectedView] ?? { title: "Nothing here", message: "" };
  ...
  }, [newTitle, createTask, selectTask]);
  ...
  return (
    <div className="h-full bg-bg-secondary flex flex-col select-none">
      ...
      <div className="ui-pane-header border-border/80">
        {showDetail ? (
          <button
            ...
            className="flex items-center gap-1 text-sm text-text-muted hover:text-text transition-colors -ml-1 px-1 py-0.5 rounded-md hover:bg-bg-muted/60"
          >
            <ChevronLeft className="h-4 w-4 stroke-[1.8] shrink-0" />
            ...
            {selectedView !== "logbook" && (
              <div className="ui-pane-header-actions ml-auto">
                <button
                  ...
                  className="flex h-7 w-7 items-center justify-center rounded-md text-text-muted hover:bg-bg-muted/80 hover:text-text transition-colors"
                >
      ...
      {showDetail ? (
        ...
        <div className="ui-scrollbar-overlay flex-1 overflow-y-auto py-1.5 px-1.5">
          {isLoading ? null : tasks.length === 0 && !isCreating ? (
